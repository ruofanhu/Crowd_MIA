...
def load_from_splits(paths, original_test_filename, model_predicted_filename):
    ...
    for path in paths:
        ...
        try:
            ...
            print("error path:", model_predicted)
        ...
        for (original_sentence, original_labels), (model_sentence, model_labels) in zip(original_test, model_predicted):
            try:
                ...
                error_count += 1
                error_test_path.append(os.path.join(path, original_test_filename))
                error_model_predicted.append(os.path.join(path, model_predicted_filename))
            if ' '.join(original_labels) != ' '.join(model_labels):
                sentence_potential_mistake_count[' '.join(original_sentence)] += 1
    print("error count:", error_count)
    for ots, ol in zip(error_test_path, error_model_predicted):
        print('error_test_path:', ots, "error_model_predicted:", ol)
